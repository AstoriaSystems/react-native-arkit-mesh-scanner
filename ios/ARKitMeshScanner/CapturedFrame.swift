...
/// Represents a captured camera frame with associated camera parameters.
/// Used for texture mapping during mesh scanning.
struct CapturedFrame {
    /// The captured image as a pixel buffer
    let image: CVPixelBuffer

    /// The camera transform matrix at capture time (world space)
    let transform: simd_float4x4

    /// Camera intrinsics matrix (focal length and principal point)
